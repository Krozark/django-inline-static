...
class TestInlineStaticfileTag:
    ...
    def test_not_safe(self, settings):
        settings.DEBUG = True
        assert isinstance(inline_static_tags.inline_staticfile('test.txt'), SafeText) is False
...
class TestInlineJavascriptTag:
    ...
    def test_content(self):
        assert (
            inline_static_tags.inline_javascript('js/module.js').strip()
            == 'var foo = {bar: 23}'
        )

    def test_is_safe(self):
        assert (
            isinstance(inline_static_tags.inline_javascript('js/module.js'), SafeText) is True
        )
...
class TestInlineStyleTag:
    ...
    def test_is_safe(self):
        assert isinstance(inline_static_tags.inline_style('css/all.css'), SafeText) is True
